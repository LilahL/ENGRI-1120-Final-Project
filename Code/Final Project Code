...
html"""
<p style="font-size:20px;">La Sala De Espera: Samantha Ceron, Amanda Burkart, Lilah Lopez, Aleks Cornforth</br>
...
# ╔═╡ 7ca236b4-c4d4-47a9-9f8e-e3fc7eeee99c
md"""
Isoprene is used to make rubber and is often found in tires, adhesives, and moulds, making it a very useful compound.  However, isoprene is challenging, and therefore expensive, to make.
"""
...
begin
	...
	#sum the the bottom streams
	input_2 = species_mass_flow_array_bottom[:,end]
end
...
# ╔═╡ ea78efbc-3d5f-4c6e-ad2e-58871324121e
# how many levels are we going to have in the separation tree?
number_of_levels_2 = 6;

# ╔═╡ a33daf23-2424-45e5-87c3-1f300a3b8e6d
# what compound are we trying to separate?
idx_target_compound_2 = find_compound_index(MODEL,:compound_name=>"co2");

# ╔═╡ 62c734cc-4da6-4fd9-9873-bfb693b5a6fe
begin

	# alias the mass flow into the sep-units
	# mass flow coming out of the mixer -
	mass_flow_into_seps_2 = input_2
	...
	# correct defaults -
	u_2[idx_target_compound_2] = θ_2
	d_2[idx_target_compound_2] = 1 - θ_2

	# let's compute the composition of the *always up* stream -
	
	# initialize some storage -
	species_mass_flow_array_top_2 = zeros(ℳ,number_of_levels_2)
	species_mass_flow_array_bottom_2 = zeros(ℳ,number_of_levels_2)

	for species_index = 1:ℳ
		value = mass_flow_into_seps_2[species_index]
		species_mass_flow_array_top_2[species_index,1] = value
		species_mass_flow_array_bottom_2[species_index,1] = value
	end
	
	for level = 2:number_of_levels_2

		# compute the mass flows coming out of the top -
		m_dot_top = mass_flow_into_seps_2.*(u_2.^(level-1))
		m_dot_bottom = mass_flow_into_seps_2.*(d_2.^(level-1))

		# update my storage array -
		for species_index = 1:ℳ
			species_mass_flow_array_top_2[species_index,level] = m_dot_top[species_index]
			species_mass_flow_array_bottom_2[species_index,level] = m_dot_bottom[species_index]
		end
	end
	
	# what is the mass fraction in the top stream -
	species_mass_fraction_array_top_2 = zeros(ℳ,number_of_levels_2)
	species_mass_fraction_array_bottom_2 = zeros(ℳ,number_of_levels_2)

	# array to hold the *total* mass flow rate -
	total_mdot_top_array_2 = zeros(number_of_levels_2)
	total_mdot_bottom_array_2 = zeros(number_of_levels_2)
	
	# this is a dumb way to do this ... you're better than that JV come on ...
	T_top_2 = sum(species_mass_flow_array_top_2,dims=1)
	T_bottom_2 = sum(species_mass_flow_array_bottom_2,dims=1)
	for level = 1:number_of_levels_2

		# get the total for this level -
		T_level_top = T_top_2[level]
		T_level_bottom = T_bottom_2[level]

		# grab -
		total_mdot_top_array_2[level] = T_level_top
		total_mdot_bottom_array_2[level] = T_level_bottom

		for species_index = 1:ℳ
			species_mass_fraction_array_top_2[species_index,level] = (1/T_level_top)*
				(species_mass_flow_array_top_2[species_index,level])
			species_mass_fraction_array_bottom_2[species_index,level] = (1/T_level_bottom)*
				(species_mass_flow_array_bottom_2[species_index,level])
		end
	end
end

# ╔═╡ 4c6384d3-59d1-4cc1-8033-b894d919b6a7
begin

	stages_2 = (1:number_of_levels_2) |> collect
	plot(stages_2,species_mass_fraction_array_top_2[idx_target_compound_2,:], linetype=:steppre,lw=2,legend=:bottomright, 
		label="Mass fraction i = PDO Tops")
	xlabel!("Stage index l",fontsize=18)
	ylabel!("Tops mass fraction ωᵢ (dimensionless)",fontsize=18)

	# make a 0.95 line target line -
	target_line_2 = 0.95*ones(number_of_levels_2)
	plot!(stages_2, target_line_2, color="red", lw=2,linestyle=:dash, label="Target 95% purity")
end

# ╔═╡ b7553d0b-07ae-46c3-8952-e591638a5df9
with_terminal() do

	# initialize some space -
	state_table = Array{Any,2}(undef, number_of_levels_2, 3)
	for level_index = 1:number_of_levels_2
		state_table[level_index,1] = level_index
		state_table[level_index,2] = species_mass_fraction_array_top_2[idx_target_compound_2, level_index]
		state_table[level_index,3] = total_mdot_top_array_2[level_index]
	end
...
# ╔═╡ aa2d2284-d093-48de-a2b3-97d702832836
md"""
### Financial Analysis
"""

# ╔═╡ e7a28f0c-d8f4-4ed2-b8f8-c74b608dad82
md"""
Net Present Value:
...
# ╔═╡ 9072bcd8-7aab-4484-acbf-bda7838d716c
# lifetime = 10 future years (period of time to payback the investment)
T = 10

# ╔═╡ 26d9a253-618f-41d7-bdc8-7f3473d3b8b2
# discount rate: risk-free interest rate (alternative investment: take $282020K and invest at the risk-free rate)
# assume constant over the payback lifetime -
discount_rate = 0.10

# ╔═╡ 31888fd8-8735-40c2-a2c6-4825ed658eb5
# Setup the cash flow array -
# we pay in year 1: $282020K (out, -ve)
# we save in years 2: 334469.44K (in,+ve)
# we save in years 3, 4,..., 10: 499729.44K (in,+ve)
CF_array = [-282020.0, 334469.44, 499729.44, 499729.44, 499729.44, 499729.44, 499729.44, 499729.44, 499729.44, 499729.44, 499729.44]

# ╔═╡ e39ee55b-f009-4cc2-b50f-4742fa90e3dd
...
# ╔═╡ 8ba5e20b-38b8-43ad-9fb3-29aca436aa62
# what is the PV?
PV = CF_array.*(1.0./discount_array)

# ╔═╡ b2e5d400-5224-452e-a706-c75087576842
md"""
Net Present Value with 10% discount rate:
"""

# ╔═╡ dad2998c-b665-4da6-acc7-82a7afe5f58d
# what is the NPV?
NPV = sum(PV)

# ╔═╡ f801bf0a-8a64-4a30-99bf-71a871fb06ce
#Calculate NPV for 1% discount rate
discount_rate_2 = 0.01

# ╔═╡ 94fc5511-4cae-47fb-8215-e270c3d2fa82
# compute the discount terms -
begin
	
	# main -
	discount_array_2 = ones(T+1)
	for time_index = 2:(T+1)
    	tmp_term = (1+discount_rate_2)^(time_index-1)
    	discount_array_2[time_index] = tmp_term
	end
...
# ╔═╡ ceb05862-5947-4ee2-9fab-fd6842721da0
# what is the PV?
PV_2 = CF_array.*(1.0./discount_array_2)

# ╔═╡ 21382d2b-b177-41f3-bd7f-a4f284c34ed5
md"""
Net Present Value with 1% discount rate:
"""

# ╔═╡ 692cec7a-69d0-415d-a304-4f98b57a03af
# what is the NPV?
NPV_2 = sum(PV_2)
...
md"""
### Conclusions
1. We could have a higher profit using a cheaper sugar such as sucrose
2. We could make more profit by purifying and selling more byproducts
3. Rather than using the downstream of a single separator to purify CO_2_, we could combine the downstream of all isoprene separators to maximize the amount of CO_2_ we can purify
"""
...
# ╔═╡ 67b80e2a-dea8-46de-ae31-cb9a878e4d85
md"""
Sigma Aldrich:
...
Uses for Isoprene:
	...
	http://polymerdatabase.com/Polymer%20Brands/Isoprene.html
"""
